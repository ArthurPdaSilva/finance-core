...
from src.models.finance_models import ContaMensal, Divida, Usuario
...
def sql_to_documents():
    ...
    for u in db.query(Usuario).all():
        ...
        docs.append(Document(page_content=text, metadata={"table": "usuarios", "id": u.id}))

    # CONTAS MENSAIS
    for c in db.query(ContaMensal).all():
        text = f"Conta mensal: {c.nome}. Valor: {c.valor} reais."
        docs.append(Document(page_content=text, metadata={"table": "contas_mensais", "id": c.id}))
    ...
    for d in db.query(Divida).all():
        ...
        docs.append(Document(page_content=text, metadata={"table": "dividas", "id": d.id}))
